...
```text
Leaflet QR  ──▶  https://amitdobry.github.io/?b=N         (the landing page, "בונים עם AI")
                    │  stores b in localStorage, shows three demo cards
                    ▼
             https://rhythm-runner-eight.vercel.app/?b=N   (the game; M6 stores b too)
                    │  play -> results -> workshop reveal
                    ▼
             https://amitdobry.github.io/?b=N#signup        (the form + WhatsApp, credited to leaflet N)
```

`b` is the leaflet batch, `1`..`6`, exactly as the landing page defines it.
The game never invents its own campaign scheme. Anything else in `?b=` is
ignored and treated as "no batch".
...
6. Anonymous funnel analytics with the `b` batch, plus a key-protected summary.
...
client/src/text/links.ts               WORKSHOP_URL and workshopLink(batch) (new)
client/src/analytics/analytics.ts      sid, batch, track() (new)
...
5. `WorkshopReveal`: appears 1.2 s after the results (fade in), below the
   ...
   לסדנה" that opens `workshopLink(batch)` in a new tab. The button is not
...
export function workshopLink(batch: string): string; // WORKSHOP_URL + (batch ? `?b=${batch}` : '') + '#signup'
```
...
- `batch`: on load read `?b=`, accept only `/^[1-6]$/`, store in
  `localStorage.rr_batch`; otherwise read the stored one; else `''`.
- `track(name: EventName, data?: Record<string, number | string | boolean>)`:
  `fetch('/api/events', { method: 'POST', keepalive: true, body: JSON.stringify({ sid, name, platform, batch, data }) })`,
...
- `events.ts`: `EVENT_NAMES` (the union above), `validateEvent(body)`:
  ...
  `{ days, byName: { [name]: { events, sessions } }, byBatch: { [batch]: { opened, played, completed, workshopClicked } } }`
...
### 6. Batch on the game side

`?b=N` on any game URL is stored as above and: sent with every event, and put
into `workshopLink`. The landing page already links to the game with `?b=N`
and its form reads `?b=N` back; nothing else is needed for credit to flow.

### Done when

- On a phone, a fresh visit to `/?b=3` reaches the results screen with taps
...
- The workshop button opens `https://amitdobry.github.io/?b=3#signup`.
...
- With `ADMIN_KEY` set in Vercel, `GET /api/events/summary?days=1` with the
  header shows batch `3` with `opened`, `played` and `completed` at 1 or more.
...
Done by the planner on 2026-09-21 in
`C:\Users\Admin\AI Workshop\App 2 - day 2-5\landing-page\index.html`: a third
card "דוגמה 03 - Rhythm Runner" linking to the game, three-column grid on
desktop, and the batch passed through as `?b=N`.

**Still to do by Amit:** publish the landing page. Create a public GitHub repo
named exactly `amitdobry.github.io`, push `index.html` to its root, enable
Pages. Until then the six printed QR codes and the game's workshop button
point at a 404. See that folder's `SETUP.md`.
